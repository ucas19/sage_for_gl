...
def list_diff_multiset(A, B):
    ...
    for x in A:
        ...
        if count_b[x_immutable_vecs] > _sage_const_0 :
            count_b[x_immutable_vecs] -= _sage_const_1 
        else:
            result.append(x)
    return result
    ...
    for lambda_sp_plus_so, k in P_mu_tensor_V_after_Pr_dic.items():
        if store_gl.exists(lambda_sp_plus_so):
            ...
            if contains_with_counts(P_mu_tensor_V_after_Pr,contains_lam_judge):
                ...
                print("")
                print("")
                lambda_judge_hash_two, lambda_judge_two = judge_mu_in_P(lambda_sp_plus_so,n,m,_sage_const_20 )
...
def show_kl_comps(P_mu_tensor_V_after_Pr,n,m,kl=_sage_const_0 ):
...
#            print(f"===={n}===key={groups[0]}")
            ...
            if removed_count == _sage_const_0 :
                ...
                elif which_mod==_sage_const_11 :
                    print("使用模:S4V")# 将输入转换为有理数列表并创建向量
                elif which_mod==_sage_const_12 :
                    print("使用模:S4V_star")# 将输入转换为有理数列表并创建向量
                elif which_mod==_sage_const_13 :
                    print("使用模:W4V")# 将输入转换为有理数列表并创建向量
                elif which_mod==_sage_const_14 :
...
def toulan(nn,mm):
            ...
            for c in setC:
                for d in setD:
                    ...
                    if condition_result:
                        ...
                        front, back,front_ctr,back_ctr,removed_count = remove_matches(front.list(),back.list())
                        if removed_count == _sage_const_1 :
                            ...
                            if not removed_count_t == _sage_const_0 :
                                print("理论不会发生")
    ...
    if not problems:
        ...
        for i in range(len(problems)):
            print(f"{i+_sage_const_1 }:{problems[i]}")
    if not who_not_same:
        print("恭喜! who_not_same为空!")
    else:
        print("不符合规律的向量:")
        for i in range(len(who_not_same)):
            print(f"{i+_sage_const_1 }:{who_not_same[i]}")
...
#                show_steps(v,lambda_judge)
    ...
    if flag ==_sage_const_1  and return_no_sums ==None and return_sums ==None:
        return False, None

    if flag == _sage_const_1  and cant_judge == [] and return_no_sums == []:
...
def find_path_vector(store_path_gl, lam, n, m , which_mod,flag=_sage_const_0 ):

    lam_key = vector(QQ,[which_mod]+lam.list())
    if store_path_gl.exists(lam_key) and flag==_sage_const_0 :
    ...
    if store_path_gl.exists(lam_key) and flag == _sage_const_1 :
        print("新结果是:")
        zhengli(results_V)
        print("旧结果是:")
...
def to_tex(weights,n,m,num_tem_dict = None):
    ...
    for v,k in weight_set_count_results_sums.items():
        ...
        for i in range(m):
            results = results + str_s[n+i] + ','
        if k > _sage_const_1 :
            print("+"+str(k)+"\\sum M_{"+results[:-_sage_const_1 ] +"}")
        else:
            print("+"+"\\sum M_{"+results[:-_sage_const_1 ] +"}")
...
def to_tex_next(weights, n, m, num_tem_dict=None, output_file=None, append_mode=False, 
    ...
    for v, k in weight_set_count.items():
        ...
        line = ""
        if k > _sage_const_1 :
            line = "+" + str(k) + "M_{" + results[:-_sage_const_1 ] + "}"
        else:
            line = "+" + "M_{" + results[:-_sage_const_1 ] + "}"
        
        if not silent_mode:
            print(line)
        section_lines.append(line[_sage_const_1 :])
    ...
    for v, k in weight_set_count_results_sums.items():
        ...
        line = ""
        if k > _sage_const_1 :
            line = "+" + str(k) + "\\sum M_{" + results[:-_sage_const_1 ] + "}"
        else:
            line = "+" + "\\sum M_{" + results[:-_sage_const_1 ] + "}"
    ...
    while True:
        ...
        if select_case==_sage_const_0 :
            ...
            for i in range(_sage_const_14 ):
                lowest_weight = which_one_lowest(lowest_module.get_module(i+_sage_const_1 ),lowest_module.basis_plus)
                print(f"{i+_sage_const_1 }----->{lowest_weight}")
            ...
            flag = auto_calc_long_time(store_gl,key1,n,m,_sage_const_0 )
            if flag:
                print("计算成功!")
        ...
        if select_case==_sage_const_13 :
            ...
            if sub_select_case == _sage_const_4 :
                ...
                if not len(lowest_weight) == _sage_const_1 :
                    ...
                    try:
                        store_gl.add_group(lowest_weight[_sage_const_0 ], weight_set)
                    except ValueError as e:
                        print(e)  # Key vector ... already exists
...
#            print(f"---------{removed_count}")
            ...
            for line in weight_set:
                ...
                typical_lambda_sp = typical_lambda_sp_plus_so[:n]
                typical_lambda_so = typical_lambda_sp_plus_so[-m:]
                results_typical = test_K_L(n,m,typical_lambda_sp,typical_lambda_so,_sage_const_1 )
...
#                    break
            ...
            abc = input("是否准备更新?")
            if abc == "yes":
                results_V = find_path_vector(store_path_gl ,atypical_lambda_sp_plus_so, n, m,which_mod,_sage_const_1  )
            else:
